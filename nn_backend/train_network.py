...
class ModelTrainer:
    ...
    def load_txt_data(path_to_file: str) -> tuple[list[str], list[float]]:
        """
        Load data from the txt file and parse them into a list of positions in FEN format
    ...
    def load_numpy_data(path_to_folder: str, name: str) -> tuple[np.ndarray, np.ndarray]:
        return (
            np.load(path_to_folder + f"/{name}_inputs.npy"),
            np.load(path_to_folder + f"/{name}_outputs.npy"),
        )

    @staticmethod
    def save_numpy_data(
        path_to_folder: str, name: str, inputs: Iterable[Any], outputs: Iterable[Any]
    ) -> None:
        """
        Save already processed data for neural network training in numpy format for fast loading.
        :param path_to_folder: path to the folder for inputs.npy and outputs.npy files
...
if __name__ == "__main__":
    ...
    # load data from a numpy file (fast)
