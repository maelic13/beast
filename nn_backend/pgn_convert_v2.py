...
class DataHelper:
        ...
        with open(file_name, encoding="UTF-8") as game_file:
            ...
            while game is not None:
                ...
                if num_games % log_after == 0:
                    cls._log_extracting_progress(file_name, start_time, local_start_time, num_games, log_after, len(positions))
                    local_start_time = time()
        cls._log_progress(file_name, start_time, local_start_time, num_games, None, len(positions))
        return positions

    @classmethod
    def _log_extracting_progress(cls, file_name: str, start_time: float, local_start_time: float, games_number: int,
                      log_games_number: Optional[int], positions_number: int) -> None:
        current_time = time()
    ...
    def evaluate(cls, positions: List[str]) -> List[Tuple[str, float]]:
        results = list()
        for result in tqdm(Pool(processes=8).imap_unordered(cls._evaluate_position, positions), total=len(positions)):
            results.append(result)
        return results

    @classmethod
    def evaluate2(cls, positions: List[str], num_processes: Optional[int] = None) -> List[Tuple[str, float]]:
        num_processes = num_processes or cpu_count()
        eval_positions: List[Tuple[str, float]] = list()
    ...
    def _evaluate_positions(cls, data: List[str]) -> List[Tuple[str, float]]:
        ...
        analysed_positions: List[Tuple[str, float]] = list()
        ...
        for position in data:
            ...
            if len(analysed_positions) % log_after == 0:
                cls._log_evaluating_progress(f"{current_process().name}", start_time, local_start_time,
    ...
    def _log_evaluating_progress(cls, process_name: str, start_time: float, local_start_time: float,
                                 positions_done: int, log_after: Optional[int], total_positions: int) -> None:
        current_time = time()
        minutes = int((current_time - start_time) / 60)
        seconds = int((current_time - start_time) % 60)
        print(process_name)
        ...
        print(f"{minutes} minutes {seconds} seconds elapsed.")
        if log_after:
            print(f"Currently {int(log_after / (current_time - local_start_time))} positions/s")
        print(f"Globally {int(positions_done / (current_time - start_time))} games/s")
        total_seconds_left = (total_positions - positions_done) / (int(log_after / (current_time - local_start_time)))
        hours_left = int(total_seconds_left / 3600)
        minutes_left = int((total_seconds_left % 3600) / 60)
        seconds_left = int(total_seconds_left % 60)
        print(f"Expected time left: {hours_left} hours {minutes_left} minutes {seconds_left} seconds.")
        print()

    @classmethod
    def _parse_data(cls, data: List[str], num_processes: int) -> List[List[str]]:
        k, m = divmod(len(data), num_processes)
        return list(data[i * k + min(i, m):(i + 1) * k + min(i + 1, m)] for i in range(num_processes))
...
if __name__ == "__main__":
    ...
    print(f"Elimination of doubles took {int((time() - start) / 60)} minutes {int((time() - start) % 60)} seconds.")
