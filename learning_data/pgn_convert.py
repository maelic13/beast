...
def parse_pgn(number):
    ...
    inp = []
    start = time.time()
    engine = chess.engine.SimpleEngine.popen_uci("C:/Users/maelic/Documents/Chessbase/Engines/stockfish_x64_bmi2.exe")
    ...
    return inp, evaluations1, evaluations2               # lists
...
def eval_to_output(evaluations1, evaluations2):
    ...
    for i in range(0,len(evaluations1)):
        try:
            single_output_d10.append(float(evaluations1[i])/2000)
            single_output_d5.append(float(evaluations2[i])/2000)
        except:
            break
    single_output_d5 = np.array(single_output_d5)
    single_output_d10 = np.array(single_output_d10)

    class_output_d5 = []
    class_output_d10 = []
    for i in range(0,len(evaluations1)):
        try:
            class_output_d10.append(eval_to_sparse_7_class(evaluations1[i]))
            class_output_d5.append(eval_to_sparse_7_class(evaluations2[i]))
        except:
            break
    class_output_d10 = np.array(class_output_d10)
    class_output_d5 = np.array(class_output_d5)

    return class_output_d5, class_output_d10, single_output_d5, single_output_d10  # numpy arrays
...
def fen_to_just_pieces_input(fen):
    ...
    input = np.zeros((6,8,8), dtype=int)
    ...
    for i in range(0,64):
        if board.turn:
            ...
            if board.piece_at(i) == chess.Piece(chess.PAWN, chess.WHITE):
                input[0,7-i//8,7-i%8] = -1
            elif board.piece_at(i) == chess.Piece(chess.PAWN, chess.BLACK):
                input[0,7-i//8,7-i%8] = 1
            elif board.piece_at(i) == chess.Piece(chess.KNIGHT, chess.WHITE):
                input[1,7-i//8,7-i%8] = -1
            elif board.piece_at(i) == chess.Piece(chess.KNIGHT, chess.BLACK):
                input[1,7-i//8,7-i%8] = 1
            elif board.piece_at(i) == chess.Piece(chess.BISHOP, chess.WHITE):
                input[2,7-i//8,7-i%8] = -1
            elif board.piece_at(i) == chess.Piece(chess.BISHOP, chess.BLACK):
                input[2,7-i//8,7-i%8] = 1
            elif board.piece_at(i) == chess.Piece(chess.ROOK, chess.WHITE):
                input[3,7-i//8,7-i%8] = -1
            elif board.piece_at(i) == chess.Piece(chess.ROOK, chess.BLACK):
                input[3,7-i//8,7-i%8] = 1
            elif board.piece_at(i) == chess.Piece(chess.QUEEN, chess.WHITE):
                input[4,7-i//8,7-i%8] = -1
            elif board.piece_at(i) == chess.Piece(chess.QUEEN, chess.BLACK):
                input[4,7-i//8,7-i%8] = 1
            elif board.piece_at(i) == chess.Piece(chess.KING, chess.WHITE):
                input[5,7-i//8,7-i%8] = -1
            elif board.piece_at(i) == chess.Piece(chess.KING, chess.BLACK):
                input[5,7-i//8,7-i%8] = 1
...
if __name__ == '__main__':
    ...
    print('Parsing PGN\n')
    number = 50000
    inp, evaluations1, evaluations2 = parse_pgn(number)

    # Evaluate positions
    print('Evaluating positions')
    class_output_d5, class_output_d10, single_output_d5, single_output_d10 = eval_to_output(evaluations1, evaluations2)

    # Save as .npy files
    print('Saving .npy files')
    np.save('C:/Users/maelic/Documents/VUT/DP/learning_data/data/input', inp)
    np.save('C:/Users/maelic/Documents/VUT/DP/learning_data/data/class_output_d5', class_output_d5)
    np.save('C:/Users/maelic/Documents/VUT/DP/learning_data/data/class_output_d10', class_output_d10)
    np.save('C:/Users/maelic/Documents/VUT/DP/learning_data/data/single_output_d5', single_output_d5)
    np.save('C:/Users/maelic/Documents/VUT/DP/learning_data/data/single_output_d10', single_output_d10)
    end = time.time()
